...
namespace ShatteredRealm.Content.Items.Weapons.Ranged.Flamethrowers
{
	public class PlantBurner : ModItem
	{
		public override void SetDefaults()
		{
			...
			// Gun Properties
			Item.shoot = ProjectileID.SporeGas; // For some reason, all the guns in the vanilla source have this.
			Item.shootSpeed = 5f; // The speed of the projectile (measured in pixels per frame.)
			Item.useAmmo = ItemID.JungleSpores; // The "ammo Id" of the ammo item that this weapon uses. Ammo IDs are magic numbers that usually correspond to the item id of one item that most commonly represent the ammo type.
		}
        ...
        {
			int AmmoReserve = 30;
			Main.LocalPlayer.GetModPlayer<ShatteredPlayer>().plantburnerConsume -= 1;
			if (Main.LocalPlayer.GetModPlayer<ShatteredPlayer>().plantburnerConsume == 0)
            {
				Main.LocalPlayer.GetModPlayer<ShatteredPlayer>().plantburnerConsume += AmmoReserve;
				return true;
			}				
			else
            {
		...
		public override bool Shoot(Player player, EntitySource_ItemUse_WithAmmo source, Vector2 position, Vector2 velocity, int type, int damage, float knockback)
		{
			...
			Projectile.NewProjectile(source, position, perturbedSpeed * 2, type, damage, knockback, player.whoAmI);
