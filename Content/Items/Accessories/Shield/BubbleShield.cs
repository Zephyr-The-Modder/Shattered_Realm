	...
	public class BubbleShield : ModItem
	{
        ...
        public override void UpdateAccessory(Player player, bool hideVisual)
        {
            if (player.GetModPlayer<ShatteredPlayer>().shieldCooldown <= 0)
            {
                if (Timer >= 14)
                {
                    Projectile.NewProjectile(Projectile.GetSource_NaturalSpawn(), player.Center, Vector2.One.RotatedByRandom(MathHelper.ToRadians(360)), ProjectileID.FlaironBubble, SRUtils.ScaleShieldEffectPower(79, player), 0, Main.myPlayer, -10);
                    Timer = 0;
                }
                Timer++;
            }
